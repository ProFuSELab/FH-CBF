...
// --------------------   --------------------    --------------------
// May 2018               Lalintha Polpitiya      All belief bruteforce computations FH and DS
//
//**************************************************************************************************
...
int main()
{
	...
	{       
		...
		for (int a = 1; a <= fod; a++)
		{
			...
			for (int t = 1; t <= a; t++)
				new_arr(0, a, t); 
			for (int arounds = 0; arounds < 1019 / fod; arounds++)
			...
			{       
				...
				{       
					...
					cond_begin = clock();
					if (b_param.size() == a)
						cond_matrix.writeBl(cond_matrix.getPower(a) - 1, 1.0);
					else
					{
						...
						cond_matrix.writeBl(belief_index, condBelief);
					}
					cond_end = clock();
